...
def create_net(args):
    ...
    elif args.dataset == 'COCO': num_classes = 40
    if args.arch == 'fc':
...
class L2L(nn.Module):
    def __init__(self, args, num_classes):
        ...
        device = 'cuda'
        # # add ------------ backbone load
        self.backbone = MobileFaceNet(512).to(device)  # embeding size is 512 (feature vector)
        self.backbone.load_state_dict(torch.load('/home/dang/experiment/Face_Attribute/light_model/Weights/MobileFace_Net', map_location=lambda storage, loc: storage))
        self.backbone.fc = torch.nn.Linear(in_features= 512, out_features= 2048 * 4 * 4, bias=True).to(device)
    ...
    def forward(self, x, target_a=None, target_b=None, lam=None):
        # x.size torch.Size([16, 3, 112, 112])
        # features.size torch.Size([16, 2048, 4, 4])
    
        #add ------------ backbone load
        # x = self.backbone(x)
        # features = self.backbone.fc(x)
        # features = features.view(-1, 2048, 4, 4)


        # print(f"features",features.size())
        # print("========")
        
        # features = self.backbone(x.cpu().numpy())
        # features = torch.from_numpy(features.numpy()).cuda()
        # features = torch.from_numpy(features)        
        
        
        # origin ---------------  
        features = self.backbone.forward_features(x)   
        pos = self.pos_embedding() #[1, 2048, 8, 8]
